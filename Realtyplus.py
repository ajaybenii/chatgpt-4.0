...
def realtyplus_news_generated(url):

    raw_headline = str(url).split("/")[-1]

    # Remove the numeric part and the `.html` extension
    headline_without_numbers = "-".join(raw_headline.split("-")[:-1]).replace(".html", "")

    # Replace hyphens with spaces and capitalize the words
    formatted_headline = headline_without_numbers.replace("-", " ").capitalize()
    ...
    def fetch_and_extract_text(url):
        ...
        try:
            ...
            text = soup.select_one('body > div.col-md-12.p-0 > div.container.mb-4.stry-mt.mob-p-0 > div > div.col-md-8.rightSidebar.mob-p-0 > div > div > div:nth-child(4) > div').get_text()
    ...
    # print(f"Create Clear and clean Image without putting any words. Ensure the theme of the image aligns with this line: {headline}.")
  ...
  ##  img.show()
    ...
    if wordpress_response.status_code == 200:
        return {"Response":"News article of Realtyplus is generaterd successfully."}
...
#     for link in links:
#         href = link.get('href')
#         # Include only URLs starting with 'news-views'
#         if href and href.startswith("news-views"):
#             full_url = "https://www.rprealtyplus.com/" + href
#             if full_url not in news_urls:  # Ensure uniqueness
#                 news_urls.append(full_url)
        
#         if len(news_urls) == 6:  # Stop after collecting 5 URLs
#             break

#     for i, news_url in enumerate(news_urls, start=1):
#         raw_headline = str(news_url).split("/")[-1]
...
#         # break
#         generate_news(news_url,i+1,formatted_headline)
